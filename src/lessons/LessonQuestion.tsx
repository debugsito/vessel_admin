...
const LessonQuestions = (props: any) => {
  ...
  const [questionUpdate, setQuestionUpdate]: any = useState(false);
  ...
  const getQuestions = () => {
    ...
    if (record && record.question_ids) {
      Object.keys(record.question_ids).forEach(async (index) => {
        ...
        tempFullQuestions[mapQuestionId] = axiosQuestions;
        ...
        axios
          ...
          .then((data: any) => {
            ...
            tempQuestions[mapQuestionId] = tempLessonQuestions;
  ...
  return (
    <>
          ...
          <TableBody>
            {loaded
              ...
              && Object.entries(lessonQuestions)
                .sort((a:any, b:any) => (a[1].question_sequence - b[1].question_sequence)).map(
                  ([key, lessonQuestion]: any) => (
                    <TableRow key={key}>
                      ...
                      <TableCell style={{ paddingTop: 5, paddingBottom: 5 }}>
                        {fullQuestions[lessonQuestion?.question_id]?.text}
                      </TableCell>
                      <TableCell
                      ...
                      <TableCell style={{ paddingTop: 5, paddingBottom: 5 }}>
                        {fullQuestions[lessonQuestion?.question_id]?.type}
                      </TableCell>
                      <TableCell
                        align="center"
                        style={{ paddingTop: 5, paddingBottom: 5 }}
      ...
      <CreateLessonQuestion
        record={lessonQuestions}
        lesson_id={record && record.id}
        onCreate={refreshLessonQuestions}
      />
      <br />
      <Dialog
        open={dialogOpen}
